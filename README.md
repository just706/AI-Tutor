...
整体规划统一维护在 [Development 的总体路线](docs/Development.md#总体路线与当前进度)。阶段 0 已验收，阶段 1 正在进行：教材问答已统一入口、保存会话教材和回答引用，并支持常见 Java 对象的连续追问；刷新后可继续提问和查看原文依据。下一项是失败重试，随后完成固定题集，再推进教学闭环、学习证据和效果验证。首个可验收版本以 Java 入门教材为试点，Python / LangGraph 等架构方案按评测结果决定是否采用。
